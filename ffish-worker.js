...
async function loadFfishModule() {
    try {
        ...
        // UMD 模块会将自身挂载到 self.Module
        ffish = self.Module || self.ffish;
        
        if (ffish) {
            console.log('[ffish Worker] ffish模块加载成功 (importScripts)');
    ...
    try {
        ...
        console.log('[ffish Worker] ffish模块加载成功 (import)');
        console.log('[ffish Worker] ffish类型:', typeof ffish);
        console.log('[ffish Worker] ffish属性:', Object.keys(ffish).slice(0, 20));
        
        return true;
    } catch (error) {
        console.error('[ffish Worker] 动态 import 加载失败:', error);
    }
